...
void AtlasCore::PrintStatisticsBlock(const char* Title, InsertionStatistics& Stats)
{
	...
	if (Stats.Name.length() > 0)
		printf("| %s: %s\n", Title, Stats.Name.c_str());
	else
		printf("| %s\n", Title);
	printf("|   Start: Line %u  File Position $%X", Stats.LineStart, Stats.StartPos);
	if (Stats.MaxBound != -1)
...
bool AtlasCore::ExecuteCommand(Command& Cmd)
{
	...
	case CMD_JMP3:
		File.MoveT(StringToUInt(Cmd.Parameters[0].Value), -1);
		Stats.NewStatsBlock(File.GetPosT(), -1, Cmd.Line, Cmd.Parameters[1].Value);
		Logger.Log("%6u JMP       ROM Position is now $%X\n", Cmd.Line, StringToUInt(Cmd.Parameters[0].Value));
		IsInJmp = true;
		return true;
	case CMD_JMP4:
		File.MoveT(StringToUInt(Cmd.Parameters[0].Value), StringToUInt(Cmd.Parameters[1].Value));
		Stats.NewStatsBlock(File.GetPosT(), StringToUInt(Cmd.Parameters[1].Value), Cmd.Line, Cmd.Parameters[2].Value);
		Logger.Log("%6u JMP       ROM Position is now $%X with max bound of $%X\n",
			Cmd.Line, StringToUInt(Cmd.Parameters[0].Value), StringToUInt(Cmd.Parameters[1].Value));
		IsInJmp = true;
		return true;
	...
	case CMD_FILL:
		FillByte = StringToUInt(Cmd.Parameters[0].Value);
		if (FillByte > 255)
		{
			Logger.ReportError(__LINE__, "Fill Byte for CMD_FILL must be between 0 and 255");
			return false;
		}
		File.EnableFillBlock(FillByte);
		Logger.Log("%6u FILL      Fill Value $%02X\n", Cmd.Line, StringToUInt(Cmd.Parameters[0].Value));
		return true;

	case CMD_FILL2:
		FillByte = StringToUInt(Cmd.Parameters[0].Value);
		Size = StringToUInt(Cmd.Parameters[1].Value);
		if (FillByte > 255)
		{
			Logger.ReportError(__LINE__, "Fill Byte for CMD_FILL must be between 0 and 255");
			return false;
		}
		File.FillT(FillByte, Size);
		Logger.Log("%6u FILL      Fill Value $%02X DataCount $%08X\n", Cmd.Line, StringToUInt(Cmd.Parameters[0].Value), StringToUInt(Cmd.Parameters[1].Value));
		return true;

	case CMD_INSERTBINARY:
		Size = File.InsertBinaryT(Cmd.Parameters[0].Value, StringToUInt(Cmd.Parameters[1].Value), StringToUInt(Cmd.Parameters[2].Value));
		if (Size > 0) // Success
		{
			Logger.Log("%6u INSERTBINARY File %s StartOffset $%08X CopiedBytes $%08X\n", Cmd.Line, Cmd.Parameters[0].Value, StringToUInt(Cmd.Parameters[1].Value), Size);
			return true;
		}
		else // Failed
			return false;
